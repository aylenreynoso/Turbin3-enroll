...
mod tests {
    ...
    fn transfer_sol() {
        let keypair = read_keypair_file("dev-wallet.json").expect("Couldn't find wallet file");
        let to_pubkey = Pubkey::from_str("6iNsdFXsYNgtTrGWqEN91oopCLwZ3corSoMZAmpeLHXr").unwrap();
    ...
    fn enroll() {
        ...
        let blockhash = rpc_client.get_latest_blockhash().expect("Failed
        to send transaction");
        ...
        let args = CompleteArgs {github: b"aylenreynoso".to_vec() };
        let transaction: Transaction = WbaPrereqProgram::complete(
